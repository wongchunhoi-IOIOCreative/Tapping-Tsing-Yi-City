...
void setup() {
  ...
  // Turn on amp
  AudioMemory(20);
  dac1.analogReference(EXTERNAL); // much louder Volume ! if external <-- need to be external
...
void loop() {
  ...
  */
  ...
  if (FmSynth == true) {
    ...
    int FMFilterFreq = 800; 

    //FM Tone A5
    if (FMTone == 1) { 
    toneFMMod = 1380;   //1380
    toneFMSine = 1000 + intensity / 1000;  //original 1000 hz
        FMFilterFreq = 6000;
    }
    ...
    mixer2.gain(0, 1); // FMsound not very signifcant even 
  }
  /* FM Synth END */
